...
def mink_extract(file_path):
    ...
    A_norm = area/max(area)
    P_norm = perimeter/max(perimeter)
    E_norm = euler/max(euler)
    
    mink = np.hstack((A_norm,P_norm,E_norm))
    
    return mink

prefixes = np.array(["OA","OB","OC","OD","OE","OF"])

file_names = np.array([])

for pref in prefixes:
    for num in np.linspace(1,7,7):
...
for file in file_names:
    ...
    if file[1]=="A" or file[1]=="B" or file[1]=="C":
        labels = np.append(labels,0)
    
    elif file[1]=="D" or file[1]=="E" or file[1]=="F":
        labels = np.append(labels,1)
    

data_array = np.array(data)
mink_1 = mink_extract('Raw_data/OD_1_mink.txt')
...
data_set =  data_set.shuffle(7).batch(3)
...
#save model
model.save(os.path.join('models','Protein_Mink_classifier1.h5'))
